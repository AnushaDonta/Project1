...
public class MedOrderDaoImp implements MedOrderDao{
	...
	public List<MedOrder> getMedOrdersByDoctorName(String dname) {
		...
		Query query=entityManager.createQuery("select m from MedOrder m where m.dname like ?1");
	...
	public List<MedOrder> getMedOrdersByDate(String date) {
		...
		Query query=entityManager.createQuery("select m from MedOrder m where m.date like ?1");
